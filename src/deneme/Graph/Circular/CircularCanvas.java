 ...
 * ama kare yerine daire. Satir (row) = merkeze uzaklik (menzil), sutun (col) =
 * aci.
...
public class CircularCanvas extends GLCanvas implements GLEventListener {
    ...
    public CircularCanvas(GLCapabilities caps, BlockingQueue<QueueMessage> queue, Simulation simulation) {
        super(caps);
    ...
    private void installCameraControls() {
        // fare konumlari kare cizim alanina gore hesaplanir (pencere daha buyuk
        // olabilir)
    ...
    private void showTargetPopup(MouseEvent e) {
        int side = Viewport.side(getWidth(), getHeight());
        ...
        float u = worldX / SCREEN_RESOLUTION;
        float v = worldY / SCREEN_RESOLUTION;
        ...
        double angle = bearing / (2.0 * Math.PI);

        if (angle < 0.0) {
            angle += 1.0;
        }

        int targetX = Math.round((float) (angle * SCREEN_RESOLUTION));
        int targetY = Math.round((float) (radius * SCREEN_RESOLUTION));

        Target target = simulation.findTargetAt(targetX, targetY);
        ...
        menu.add(new JMenuItem("Tur: " + target.getType().getTypeName()));
        menu.add(new JMenuItem("ID: " + (target.isHasID() ? target.getID() : "Yok")));
        menu.add(new JMenuItem(String.format("Gain: %.2f", target.getGainFactor())));
    ...
    private class RowConsumer extends MessageConsumer {
        RowConsumer(BlockingQueue<QueueMessage> queue) {
            super(queue);
        }
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        // viewport kare oldugu icin ayrica aspect duzeltmesi gerekmiyor (cember
        // yuvarlak kalir)
    ...
    /**
     * Sol ustte, haritanin 1/4 olcekli aynisi: ayni polar gorsel + halkalar,
     * gridsiz.
     */
    private void drawMinimap(GL2 gl) {
        if (!minimap.begin(gl, viewport))
            return;
